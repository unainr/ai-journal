...
export default function AIFloatingBar({
  editor,
  onUpgrade, // ✅ lifted up to JournalEditor
}: {
  editor: Editor | null;
  ...
  const updatePosition = useCallback(() => {
    ...
    setPos({
      ...
      left: Math.min(Math.max(0, rect.left - editorRect.left), editorRect.width - 480),
    });
    setVisible(true);
  }, [editor]);
  ...
  const handleAction = async (action: AIAction) => {
    ...
    if (!canUseAI) {
      onUpgrade("AI writing is limited on the free plan. Upgrade to Pro for full access."); // ✅
      return;
    }
    ...
    if (action === "continue") {
      editor.chain().focus().insertContentAt(to, " " + result.data).run();
    } else {
      editor.chain().focus().deleteRange({ from, to }).insertContentAt(from, result.data).run();
    }
